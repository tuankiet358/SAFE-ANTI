"""
cogs/anti/cog.py — Class Cog chính của module SAFE-ANTI 2.0.

Chứa toàn bộ Event Listeners và Commands:
  Listeners:
    - on_ready                    : Snapshot cache kênh khi bot sẵn sàng
    - on_automod_action_execution : Log vi phạm AutoMod đa giai đoạn
    - on_message                  : Fallback Anti-Spam/Link + Mass Mention + Emoji Spam
    - on_member_join              : Account Age Filter + Anti-Bot Raid
    - on_webhooks_update          : Anti-Webhook — xóa webhook trái phép
    - on_guild_channel_update     : Anti-Channel Rename — khôi phục tên/topic
    - on_guild_channel_create     : Cập nhật cache kênh mới
    - on_guild_channel_delete     : Anti-Nuke channel delete + dọn cache
    - on_member_ban               : Anti-Nuke ban hàng loạt

  Commands (slash + prefix):
    - /anti_on  | !anti_on  : Mở bảng điều khiển BẬT hệ thống Anti
    - /anti_off | !anti_off : Mở bảng điều khiển TẮT hệ thống Anti
"""
...
from .constants import (
    _BRAND, _DIVIDER,
    _CLR_SUCCESS, _CLR_DANGER, _CLR_ALERT, _CLR_PURGE,
    _CLR_PHASE1, _CLR_PHASE2, _CLR_BYPASS,
    _CLR_AUTOMOD, _CLR_BOT_RAID, _CLR_MENTION,
    _CLR_AGE, _CLR_WEBHOOK, _CLR_EMOJI, _CLR_RENAME,
    _ICO_SHIELD, _ICO_LINK, _ICO_CHAT, _ICO_STOP,
    _ICO_TICK, _ICO_RESULT, _ICO_TIME, _ICO_LOCK,
    _AUTOMOD_RULE_ANTILINK, _AUTOMOD_RULE_ANTISPAM,
    _MASS_MENTION_THRESHOLD, _MASS_MENTION_TIMEOUT_MIN,
    _ACCOUNT_AGE_DAYS, _EMOJI_SPAM_THRESHOLD, _EMOJI_TIMEOUT_MIN,
)
from .helpers import (
    _get_log_channel,
    _safe_send,
    _notify_violation_channel,
    _build_anti_status_embed,
    _build_cmd_perm_denied_embed,
    _is_admin,
    _count_emoji,
)
from .ui import AntiOnView, AntiOffView
